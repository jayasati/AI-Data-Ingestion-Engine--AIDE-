import type { ApiResponse, DatasetPreviewResponse } from "@aide/shared-types";
 ...
 */
export async function submitPreview(file: File): Promise<DatasetPreviewResponse> {
  const formData = new FormData();
  formData.append("file", file);

  let response: Response;
  try {
    response = await fetch(`${env.apiUrl}/preview`, { method: "POST", body: formData });
  } catch (cause) {
    throw new ApiClientError("NETWORK_ERROR", "Could not reach the AIDE API.", { cause });
  }

  let body: ApiResponse<DatasetPreviewResponse>;
  try {
    body = (await response.json()) as ApiResponse<DatasetPreviewResponse>;
  } catch {
